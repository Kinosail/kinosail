...
| --- | --- | --- |
| `/media` | Media Files and Subtitle Files | writable for atomic sidecar creation |
...
The Server scans the Media Library mount and writes validated sidecars and stores normalized catalog state. A Viewer request is authorized by the same application operation whether it comes from the web app, `/api/v1`, a supported Jellyfin flow, or an optional MCP connection.
...
- SubDL, OpenSubtitles.com, and SubSource can search for subtitles when their credentials are configured. Kinosail sends bounded media identity, filename, language, and release-hash data. It does not upload media.
- SubSource requires personal-use acceptance. Its downloads stay inside the household and remain unchanged.
...
Outbound integration URLs are validated. Kinosail follows only explicitly allowlisted integration redirects and rejects unsafe network destinations. Integration providers can still receive the data present in the request. Review each provider's terms before enabling it.
