...
| Private management | Individually paired Owner devices, with two-step sign-in | Forward UDP `51822` in the standard installation |
...
2. Open **Settings → Manage while away** on your home network. Enter your public Server hostname and UDP port, normally `your-server.duckdns.org:51822`, and choose **Enable private management**. A valid HTTPS certificate must already be available; if issuance is still starting, wait and try again.
3. In the router, forward **UDP 51822** to **UDP 51822** on the Server's reserved LAN IP. This is separate from the TCP `443` rule used for viewing. The standard container maps this host port to internal UDP `51821`. A custom external UDP port may be any port from `1024` through `65535`; use the exact mapping shown on the management page.
