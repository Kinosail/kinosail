...
## Why Jellyfin apps require Trusted HTTPS
...
Save Trusted HTTPS in **Settings → Access**. Enable Jellyfin compatibility in **Advanced configuration**, then restart Kinosail once.
...
**Settings → Access** supports two providers. New setups select DuckDNS because it has the easiest setup.
...
Sign in as an Owner. Open **Settings → Access** and note the saved Trusted HTTPS hostname.

The Connect address uses the saved Trusted HTTPS hostname and the Kinosail Subtitles port. Keep `https://` and the port.

The address has this form:

```text
https://myhome.dedyn.io:38128
```

Use your saved hostname and configured port. Do not change the address to `http://`. Do not use the public remote-access address unless you configured remote access separately.
...
5. Enable `integrations.jellyfin.enabled`.
6. Restart Kinosail once.
7. Open Kinosail at the new trusted address.
8. Form the Connect address from the saved hostname and Kinosail Subtitles port.
...
Confirm that the device is on the same network. Use the hostname from **Settings → Access** with the Kinosail Subtitles port. Confirm that Jellyfin support is enabled.

Keep HTTPS and the configured port. Confirm that Trusted HTTPS reports ready after the restart.

### The app says the certificate is invalid

The device reached the wrong address or Kinosail has not completed Trusted HTTPS setup. Use the saved hostname and configured port. Check the status.
