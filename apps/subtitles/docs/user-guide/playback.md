...
If the needed subtitle is not present, an Owner can use **Find** in **Playback & downloads** when a subtitle provider is configured. The provider request leaves the Server; the media file does not.
