...
test("passwords can be shown and hidden without changing their value", async ({ page }) => {
...
test("the password reveal control stays aligned across responsive settings", async ({ page }, testInfo) => {
	await page.goto("/login");
	await page.getByLabel("Name").fill("Owner");
