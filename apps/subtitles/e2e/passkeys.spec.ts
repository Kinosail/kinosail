...
const mockOrigin = process.env.KINOSAIL_E2E_URL ?? "http://localhost:38128";
...
test("a successful explicit passkey sign-in reaches the requested page", async ({ page }) => {
	const origin = await mockPasskeyPage(page, mockOrigin, () => ({ status: 200, contentType: "application/json", body: '{"publicKey":{}}' }), `<body data-passkey-waiting="Waiting for your passkey…"><input autocomplete="username webauthn"><button data-passkey-login>Sign in with passkey</button><output data-passkey-status></output><script src="/static/passkeys.js"></script></body>`);
...
test("a verified passkey reports success while the destination is still loading", async ({ page }) => {
	const origin = await mockPasskeyPage(page, mockOrigin, () => ({ status: 200, contentType: "application/json", body: '{"publicKey":{}}' }), `<body><input autocomplete="username webauthn"><button data-passkey-login>Sign in with passkey</button><output data-passkey-status></output><script src="/static/passkeys.js"></script></body>`);
	await page.addInitScript(() => { (window as typeof window & { passkeySucceeds?: boolean }).passkeySucceeds = true; });
	let release!: () => void;
