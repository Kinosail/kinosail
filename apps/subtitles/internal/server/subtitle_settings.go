...
const subtitleCleanupSettingsHTML = `<section id="cleanup"><h2>Remove unwanted subtitle files</h2><p>Keep the languages you use so playback has fewer choices. When subtitles are on, Kinosail selects the first available preferred language.</p><p>Cleanup is optional and off until you enable it. Preview tagged .srt and .vtt files before deletion. Embedded tracks and files with uncertain language stay in place.</p><form action="/settings/subtitles/cleanup" method="get"><label><input type="checkbox" name="enabled" value="on" required> Enable subtitle language cleanup</label><label>Languages to keep<select name="language" multiple size="6" required aria-describedby="cleanup-language-help">{{range .AllLanguages}}<option value="{{.Tag}}" {{if .Selected}}selected{{end}}>{{.Name}} ({{.Tag}})</option>{{end}}</select></label><p id="cleanup-language-help">Select at least one language. On a keyboard, hold Command or Ctrl to select several.</p><label>Forced subtitles in every language<select name="forced"><option value="keep">Keep</option><option value="delete">Delete</option></select></label><button>Preview files to delete</button></form></section>`
