...
<div class="primary-player-actions"><div class="primary-action-row"><form action="/list/{{.ID}}" method="post"><button name="listed" value="{{if .Listed}}false{{else}}true{{end}}">{{if .Listed}}Remove from{{else}}Add to{{end}} My List</button></form>{{if ne .Kind "photo"}}<form action="/watched/{{.ID}}" method="post"><button class="quiet" name="watched" value="{{if .Watched}}false{{else}}true{{end}}">Mark {{if .Watched}}{{t "unwatched"}}{{else}}{{t "watched"}}{{end}}</button></form>{{end}}</div>{{if or .ModeURL .CanDownload .CanFetchSubtitles}}<details class="more-player-actions"><summary>Playback &amp; downloads</summary><div>{{if .ModeURL}}<a class="mode" href="{{.ModeURL}}" aria-label="{{t .ModeLabel}}"><span>Playback</span><strong>{{t .ModeLabel}}</strong></a>{{end}}{{if .CanDownload}}<a class="mode" href="/download/{{.ID}}" aria-label="Download original"><span>Download</span><strong>Original file</strong></a>{{if .OfflineQuality}}<form action="/offline/{{.ID}}" method="post"><button class="quiet" name="quality" value="{{.OfflineQuality}}" aria-label="Prepare {{.OfflineQuality}} offline"><span>Offline</span><strong>Prepare {{.OfflineQuality}}</strong></button></form><a class="mode" href="/offline-downloads" aria-label="Offline downloads"><span>Offline</span><strong>Manage downloads</strong></a>{{end}}{{end}}{{if .CanFetchSubtitles}}<form action="/subtitles/{{.ID}}/fetch" method="post"><button class="quiet"><span>Subtitles</span><strong>{{t "Find"}} {{.SubtitleLanguage}}</strong></button></form>{{end}}</div></details>{{end}}</div>
...
{{if gt (len .Audio) 1}}<details class="player-disclosure audio-tracks"><summary><span>Audio tracks</span><small>{{len .Audio}} available</small></summary><div>{{range .Audio}}<a class="mode" href="?compatible=1&amp;audio={{.Index}}">{{.Label}}</a>{{end}}</div></details>{{end}}
...
{{if ne .Kind "photo"}}<details class="playback-tools player-disclosure"><summary><span>Watch together</span><small>Devices and shared rooms</small></summary><div class="player-actions"><button class="quiet" type="button" data-cast>Play on device</button><span role="status" aria-live="polite" data-cast-state>Available devices use a direct connection to this Server.</span></div>{{if .Room}}<div class="player-actions"><a class="mode" href="/room/{{.Room}}">Invite link</a><span role="status" aria-live="polite" data-room-state>{{if .RoomLeader}}You lead this room{{else}}Following the room leader{{end}}</span>{{if .RoomLeader}}<label>Play <select data-room-choice>{{range .RoomItems}}<option value="{{.ID}}" {{if eq .ID $.ID}}selected{{end}}>{{.Title}}</option>{{end}}</select></label>{{end}}</div>{{else}}<form class="player-actions" action="/watch-together" method="post" data-room-create><input type="hidden" name="media" value="{{.ID}}"><input type="hidden" name="seconds" value="0"><button class="quiet">Start Watch Together</button></form>{{end}}</details>{{end}}
{{if .Cast}}<section class="cast-section"><header><span class="eyebrow">People</span><h2>Cast</h2></header><div class="grid rail">{{range $index, $person := .Cast}}<article class="card">{{if $person.Image}}<img class="poster" src="/person/{{$.ID}}/{{$index}}" alt="{{$person.Name}}" loading="lazy">{{else}}<div class="poster">●</div>{{end}}<h2>{{$person.Name}}</h2>{{if $person.Role}}<small>{{$person.Role}}</small>{{end}}</article>{{end}}</div></section>{{end}}
...
func playerTemplate(template string) string {
	...
	template = strings.Replace(template, `/static/player.js?v=34`, `/static/player.js?v=53`, 1)
...
func registerPlayer(mux *http.ServeMux, index *libraryIndex, progress *progressStore, settings *settingsStore, lists *listStore, hls *hlsManager, probe *mediaProbe, subtitles *subtitleProvider, metadata *metadataStore, rooms *watchRoomAdapter, auth *authentication) {
	...
	mux.HandleFunc("GET /watch/{id}", watch(index, progress, settings, lists, probe, subtitles, metadata, rooms))
...
func watch(index *libraryIndex, progress *progressStore, settings *settingsStore, lists *listStore, probe *mediaProbe, subtitles *subtitleProvider, metadata *metadataStore, rooms *watchRoomAdapter) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		data := buildPlayerData(request, item, index, progress, settings, lists, probe, subtitles, metadata)
...
func buildPlayerData(request *http.Request, item library.Item, index *libraryIndex, progress *progressStore, settings *settingsStore, lists *listStore, probe *mediaProbe, subtitles *subtitleProvider, metadata *metadataStore) playerData { //nolint:cyclop,gocognit // The player projection assembles each supported playback capability once.
	...
	data.Tracks = playbackSubtitles(item, media, subtitles, data.SubtitleLanguage, data.DefaultSubtitles)
