...
func TestPlaybackTraceCorrelatesBrowserAndMediaWithoutPrivateData(t *testing.T) { //nolint:cyclop // One integration flow checks correlation and privacy together.
	media := t.TempDir()
	...
	logs := output.String()
	if eventResponse.Code != http.StatusNoContent || mediaResponse.Code != http.StatusPartialContent || strings.Count(logs, `"playback_session":"`+session[1]+`"`) < 3 || !strings.Contains(logs, `"msg":"playback planned"`) || !strings.Contains(logs, `"msg":"playback trace"`) || !strings.Contains(logs, `"event":"play-rejected"`) || !strings.Contains(logs, `"detail":"control:NotAllowedError"`) || !strings.Contains(logs, `"method":"direct"`) || !strings.Contains(logs, `"direct_type":`) || !strings.Contains(logs, `"request_range":"bytes=0-0"`) || !strings.Contains(logs, `"response_range":"bytes 0-0/5"`) || !strings.Contains(logs, `"response_bytes":1`) {
