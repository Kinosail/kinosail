...
func TestSubDLKeyChangesTakeEffectWithoutRestart(t *testing.T) { //nolint:cyclop,gocognit,funlen // One public journey covers save, rejection, replacement, reset, and active credentials.
	t.Parallel()
...
func TestOpenSubtitlesAndSubSourceKeysChangeWithoutRestart(t *testing.T) { //nolint:cyclop,gocognit,funlen // One public journey covers atomic credential changes and retry recovery for both providers.
	t.Parallel()
	remote := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
