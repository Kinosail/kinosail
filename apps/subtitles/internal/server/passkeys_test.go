...
func TestPasskeyCeremoniesAreLocalAndServerSide(t *testing.T) {
	...
	if account.Code != http.StatusOK || !strings.Contains(account.Body.String(), "Add a passkey") || !strings.Contains(account.Body.String(), "Add one directly on another device") || !strings.Contains(account.Body.String(), "/static/app.css?v=impeccable-1") || !strings.Contains(account.Body.String(), "/static/passkeys.js?v=14") {
	...
	if prompt.Code != http.StatusOK || !strings.Contains(prompt.Body.String(), "Protect the Owner account") || !strings.Contains(prompt.Body.String(), "Choose a sign-in method") || strings.Contains(prompt.Body.String(), "Not now") {
...
func TestPasskeyLoginBeginIsPublic(t *testing.T) {
	t.Parallel()
	servertest.PasskeyLoginBeginIsPublic(t, New(Config{DataDir: t.TempDir(), RequireAuth: true}), "https://localhost:38128")
}

func TestPasskeyLoginIgnoresStaleSameOriginSessionCookie(t *testing.T) {
	t.Parallel()
	const origin = "https://localhost:38128"
...
func TestPasskeyBeginRedirectsToConfiguredOrigin(t *testing.T) {
	...
	servertest.PasskeyBeginRedirectsToConfiguredOrigin(t, New(Config{DataDir: t.TempDir(), RequireAuth: true, AuthURL: "https://media.example:38128", Configuration: configured}), "https://media.example:38128", "https://192.0.2.55:38128")
}
