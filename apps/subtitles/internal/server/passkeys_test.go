...
func TestPasskeyCeremoniesAreLocalAndServerSide(t *testing.T) {
	...
	if account.Code != http.StatusOK || !strings.Contains(account.Body.String(), "Add a passkey") || !strings.Contains(account.Body.String(), "Add one directly on another device") || !strings.Contains(account.Body.String(), "/static/app.css?v=cinema-2") || !strings.Contains(account.Body.String(), "/static/passkeys.js?v=14") {
