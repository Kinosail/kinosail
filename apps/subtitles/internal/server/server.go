...
{{if .TMDB}}<footer><a href="https://www.themoviedb.org">TMDB</a> · This product uses the TMDB API but is not endorsed or certified by TMDB.</footer>{{end}}</main>{{languagePicker}}<script src="/static/supporter.js?v=8"></script></body></html>
{{define "homeShelfCard"}}<a class="card recent-card{{if .Stacked}} stacked{{end}}" href="{{.Href}}"{{if .Stacked}} aria-label="{{.Title}}, {{.Count}} {{.StackLabel}}; open show"{{end}}>{{if .Stacked}}<span class="poster recent-stack" aria-hidden="true"><span class="recent-stack-layer"></span><span class="recent-stack-layer"></span>{{if .ArtworkID}}<img src="/art/{{.ArtworkID}}" alt="" width="400" height="600" decoding="async" {{if .Priority}}fetchpriority="high"{{else}}loading="lazy"{{end}}>{{else}}<span class="recent-stack-placeholder">{{icon .PlaceholderIcon}}</span>{{end}}<span class="recent-stack-count">{{.Count}}</span></span>{{else}}{{if .ArtworkID}}<img class="poster" src="/art/{{.ArtworkID}}" alt="" width="400" height="600" decoding="async" {{if .Priority}}fetchpriority="high"{{else}}loading="lazy"{{end}}>{{else}}<div class="poster">{{icon .PlaceholderIcon}}</div>{{end}}{{end}}<h2>{{.Title}}</h2><small>{{.Meta}}</small></a>{{end}}
...
func newApplication(config Config) http.Handler { //nolint:funlen,cyclop,gocognit // The composition root validates defaults and wires every adapter to shared application services.
	...
	mux.HandleFunc("GET /static/subtitle-inspector.js", serveScript(subtitleInspectorJS))
	...
	registerPlayer(mux, index, progress, settings, lists, hls, probe, subtitles, metadata, rooms, auth)
	registerMarkerAdmin(mux, auth, index, probe, markers)
	frames.register(mux)
	subtitles.register(mux, auth)
	subtitleManager.register(mux, auth)
	...
	if config.SubtitleApp {
		mux.Handle("GET /{$}", auth.owner(http.HandlerFunc(subtitleManager.dashboard)))
	} else {
