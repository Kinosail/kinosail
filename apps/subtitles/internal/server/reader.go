...
func apiReaderProgress(index *libraryIndex, progress *progressStore) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		if request.Method == http.MethodGet {
			writeJSON(writer, map[string]int{"page": progress.ReaderPage(request, book.ID, len(book.Pages)), "total": len(book.Pages)}, http.StatusOK)
			return
		}
		var input struct {
			Page int `json:"page"`
		...
		if err := progress.SetReaderPage(request, book.ID, input.Page, len(book.Pages)); err != nil {
			apiError(writer, err, http.StatusBadRequest)
			return
		}
		writeJSON(writer, map[string]int{"page": input.Page, "total": len(book.Pages)}, http.StatusOK)
	}
}
