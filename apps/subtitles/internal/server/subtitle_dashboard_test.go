...
func TestSubtitleAppShowsCoverageAndWantedFiles(t *testing.T) { //nolint:cyclop // One fixture verifies the three related dashboard projections.
	...
	for _, expected := range []string{`class="library-page subtitle-app subtitle-dashboard"`, `class="app-header"`, `class="brand-lockup"`, `class="web-beta-badge">Beta`, `href="/supporter" aria-label="Support Kinosail"`, `href="/?view=summary"`, `aria-current="page"`, `/static/app.css?v=cinema-4`} {
