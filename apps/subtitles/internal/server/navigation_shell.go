...
const applicationCSSVersion = "cinema-5"
...
func withApplicationShell(settings *settingsStore, updates *updateChecker, pattern func(*http.Request) string, next http.Handler) http.Handler { //nolint:cyclop // Browser shell routing applies one ordered policy.
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		matched := pattern(request)
		if matched == "" && request.Method == http.MethodGet && !strings.HasPrefix(request.URL.Path, "/api/") && !strings.HasPrefix(request.URL.Path, "/static/") {
			localizedPageError(writer, request, "Page not found", "That Subtitles page does not exist. Check the address or return to Subtitles.", http.StatusNotFound)
			return
		}
		...
		data := applicationNavigationData{ServerName: settings.serverName(), Owner: viewer.Owner, ViewerName: viewer.Name, CanLogout: viewer.ID != "local-owner", View: applicationShellView(matched), Sort: "title", UpdateAvailable: updates != nil && updates.Available()}
...
func applicationShellRoute(pattern string) bool {
	switch pattern {
	case "GET /account", "GET /album/{id}", "GET /book/{id}", "GET /collection/{name}", "GET /metadata/bulk", "GET /offline-downloads", "GET /playlist/{name}", "GET /quick-connect", "GET /settings", "GET /settings/agent-connections", "GET /settings/backups", "GET /settings/configuration", "GET /settings/media-shares", "GET /settings/remote-readiness", "GET /settings/management", "GET /settings/system", "GET /show/{id}", "GET /supporter":
		return true
	}
	return false
}
...
func applicationShellView(pattern string) string {
	if strings.HasPrefix(pattern, "GET /settings") {
		return "settings"
	}
	switch pattern {
	case "GET /album/{id}":
...
func injectApplicationShell(page, navigation []byte) []byte { //nolint:cyclop // Template injection validates each required shell boundary.
	if start := bytes.Index(page, []byte(`/static/app.css?v=`)); start >= 0 {
		...
		if end := bytes.IndexByte(page[version:], '"'); end >= 0 {
			page = append(append(append([]byte(nil), page[:version]...), []byte(applicationCSSVersion)...), page[version+end:]...)
		}
	}
	...
	if class := bytes.Index(body, []byte(`class="`)); class >= 0 {
		...
		page = append(append(append([]byte(nil), page[:class]...), []byte("library-page subtitle-app subtitle-dashboard ")...), page[class:]...)
		bodyEnd += len("library-page subtitle-app subtitle-dashboard ")
	} else {
		page = append(append(append([]byte(nil), page[:bodyEnd]...), []byte(` class="library-page subtitle-app subtitle-dashboard"`)...), page[bodyEnd:]...)
		bodyEnd += len(` class="library-page subtitle-app subtitle-dashboard"`)
	}
