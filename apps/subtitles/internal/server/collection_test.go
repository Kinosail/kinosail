...
func TestOwnerCanCreateAndCurateCollection(t *testing.T) { //nolint:cyclop,funlen // One lifecycle proves validation, persistence, and both adapters.
	...
	watches := regexp.MustCompile(`/watch/([a-f0-9]+)`).FindAllStringSubmatch(home.Body.String(), -1)
	...
	mustContainAll(t, collection.Body.String(), "2 items", "Dune Guide", "Remove", "Delete Favorites?", "Delete Collection")
