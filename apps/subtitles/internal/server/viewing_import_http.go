...
var (
	...
	viewingImportOnboardingView = newLocalizedTemplate("viewing-import-onboarding", ignoreNonPasswordSecretAutofill(viewingImportControlHTML+`{{define "onboardingProgress"}}`+onboardingProgressHTML+`{{end}}`+updateChoicePage(strings.Replace(strings.Replace(strings.Replace(viewingImportOnboardingHTML, `/static/app.css?v=electric-1`, `/static/app.css?v=electric-1`, 1), `{{template "onboardingProgress"}}`, `{{template "onboardingProgress" .Step}}`, 1), `{{range .}}`, `{{range .Profiles}}`, 1))))
)
