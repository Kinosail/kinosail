...
func TestPWAHandlesServiceWorkerRegistrationFailure(t *testing.T) {
	...
	if !strings.Contains(script, `serviceWorker.register("/service-worker.js?v=39").then`) || !strings.Contains(script, `.catch(() => {});`) {
...
func TestOfflineDownloadClientAndWorkerExposeLocalPlaybackContract(t *testing.T) {
	...
	if !strings.Contains(worker.Body.String(), `const cacheName = "kinosail-shell-v28"`) || !strings.Contains(worker.Body.String(), `fetch(event.request).catch(() => caches.match("/offline"))`) {
