...
func hlsPolicy() playback.HLSRecipePolicy {
	return playback.HLSRecipePolicy{MaxBitrate: 100_000_000, OffsetStepMilliseconds: 30_000, StartPresentationAtZero: true}
}

func sharedHLSRecipe(recipe hlsRecipe) playback.HLSRecipe {
	return playback.HLSRecipe{OutputTime: recipe.outputTime, Width: recipe.width, Height: recipe.height, SubtitleOrdinal: recipe.subtitleOrdinal, SubtitlePath: recipe.subtitlePath, SubtitleTime: recipe.subtitleTime, Mode: recipe.mode, Burn: recipe.burn, Codec: recipe.codec, Audio: recipe.audio, Subtitle: recipe.subtitle, ToneMap: recipe.toneMap, MaxBitrate: recipe.maxBitrate, Omitted: recipe.omitted, Offset: recipe.offset}
}
