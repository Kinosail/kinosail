...
var configurationValidators = map[string]func(string) error{
	"binaries.tesseract":                   validateLocalSubtitleBinary,
	"binaries.whisper":                     validateLocalSubtitleBinary,
	"subtitles.transcription_model":        optionalValid(func(raw string) bool { return validLocalSubtitlePath(raw) && filepath.IsAbs(raw) }, "must be an absolute local model path"),
	"subtitles.transcription_architecture": allowed("tiny", "tiny.en", "base", "base.en", "small", "small.en", "medium", "medium.en", "large.v1", "large.v2", "large.v3", "large.v3.turbo"),
	...
	"integrations.subdl.url":               validateProviderURL,
	"integrations.opensubtitles.url":       validateProviderURL,
	"integrations.subsource.url":           validateProviderURL,
	"integrations.subdl.api_key":           validateProviderCredential,
	"integrations.opensubtitles.api_key":   validateProviderCredential,
	"integrations.opensubtitles.username":  validateProviderCredential,
	"integrations.opensubtitles.password":  validateProviderCredential,
	"integrations.subsource.api_key":       validateProviderCredential,
	...
	"subtitles.language":                   requiredValid(func(raw string) bool { return raw == "" || validSubtitleLanguage(raw) }, "must be a supported BCP 47 subtitle language"),
...
func validateProviderURL(raw string) error {
	if raw != "" && (len(raw) > 2048 || !federation.ValidIdentityURL(raw)) {
		return errors.New("must be a trusted provider URL")
	}
	return nil
}

func validateProviderCredential(raw string) error {
	if raw != "" && !validSubtitleProviderCredential(raw) {
		return errors.New("must contain no more than 4096 non-whitespace-bounded characters")
	}
	return nil
}
...
func validLocalSubtitlePath(raw string) bool {
	return raw != "" && len(raw) <= 4096 && utf8.ValidString(raw) && raw == strings.TrimSpace(raw) && !strings.HasPrefix(raw, "-") && !strings.Contains(raw, "://") && strings.IndexFunc(raw, unicode.IsControl) < 0
}
func validateLocalSubtitleBinary(raw string) error {
	if !validLocalSubtitlePath(raw) || strings.ContainsAny(raw, "/\\") && !filepath.IsAbs(raw) {
		return errors.New("must be an executable name or absolute local path")
	}
	return nil
}
