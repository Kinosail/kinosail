...
image="ghcr.io/kinosail/kinosail-subtitles:$version"
KINOSAIL_IMAGE="$image" "${project[@]}" pull
digest="$("${compose[0]}" image inspect --format '{{index .RepoDigests 0}}' "$image")"
if [[ ! "$digest" =~ ^ghcr\.io/kinosail/kinosail-subtitles@sha256:[a-f0-9]{64}$ ]]; then
  echo "pulled image did not resolve to an expected Kinosail digest" >&2
  exit 1
fi
if [[ "$version" == latest ]]; then
  cosign verify --certificate-identity-regexp '^https://github\.com/Kinosail/kinosail/\.github/workflows/subtitles-release\.yml@refs/tags/subtitles-v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$' --certificate-oidc-issuer https://token.actions.githubusercontent.com "$digest" >/dev/null
else
...
if [[ -z "$healthy" ]]; then
	"${project[@]}" logs --tail 50 kinosail >&2
