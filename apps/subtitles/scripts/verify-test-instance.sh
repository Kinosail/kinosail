...
grep -Fq 'Subtitle overview' <<<"$home"
grep -Fq 'Missing preferred subtitles' <<<"$home"
grep -Fq '<meter' <<<"$home"
grep -Fq 'Kinosail Subtitles' <<<"$home"

wanted="$(curl --fail --silent --insecure --cookie "$fixture/cookies" "$base/?view=wanted")"
grep -Fq 'Wanted subtitles' <<<"$wanted"
grep -Fq 'Missing preferred subtitles' <<<"$wanted"
