...
totp_code() {
  ...
  secret_hex="$(printf %s "$secret" | "$engine" run --rm --interactive --entrypoint base32 localhost/kinosail-subtitles:dev --decode | xxd -p -c 256)"
...
prepare() {
  ...
  if [[ "${KINOSAIL_TEST_IMAGE_READY:-}" != "1" ]]; then
    "$engine" build --file "$repo/apps/subtitles/Containerfile" --tag localhost/kinosail-subtitles:dev "$repo"
  fi
  "${compose[@]}" build kinosail
  staging="$(mktemp -d "$KINOSAIL_TEST_ROOT/media.XXXXXX")"
  KINOSAIL_TEST_IMAGE=localhost/kinosail-subtitles:dev CONTAINER_ENGINE="$engine" ./scripts/generate-test-media.sh "$staging"
...
case "$action" in
  ...
  browser)
    ...
    KINOSAIL_TEST_INSTANCE=1 KINOSAIL_UI_FIXTURE_DIR="$fixture_dir" KINOSAIL_TEST_TOTP_SECRET="$(<"$KINOSAIL_TEST_ROOT/totp-secret")" KINOSAIL_E2E_URL="$(url)" KINOSAIL_E2E_OUTPUT_DIR="$KINOSAIL_TEST_ROOT/playwright-results" pnpm --dir e2e test subtitle-dashboard.spec.ts --workers="${KINOSAIL_E2E_WORKERS:-1}"
    ;;
  playback)
    fixture_dir="$KINOSAIL_TEST_ROOT/ui-fixtures"
