...
grep -Fq "name: \"\${KINOSAIL_PROJECT_NAME:-kinosail-subtitles}\"" "$fixture/app/compose.release.yaml"
# shellcheck disable=SC2016 # The literal Compose interpolation is the assertion target.
grep -Fq 'image: "${KINOSAIL_IMAGE:-ghcr.io/mikeo7/kinosail-subtitles:latest}"' "$fixture/app/compose.release.yaml"
...
grep -Fq 'timeout: 15s' "$fixture/app/compose.release.yaml"
...
case "$*" in
  ...
  *"config --images"*) echo "ghcr.io/mikeo7/kinosail-subtitles:latest" ;;
  *"image inspect"*) echo "ghcr.io/mikeo7/kinosail-subtitles@sha256:${KINOSAIL_INSTALL_TEST_DIGEST:-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa}" ;;
  ...
  *"up --detach"*)
    ...
    if [[ -n "${KINOSAIL_INSTALL_TEST_FAIL_DIGEST:-}" ]] && grep -q "sha256:$KINOSAIL_INSTALL_TEST_FAIL_DIGEST" .env; then printf migrated >"$KINOSAIL_INSTALL_TEST_DATA"; fi ;;
...
PATH="$fixture/bin:$PATH" "$fixture/app/scripts/uninstall.sh" >/dev/null
...
grep -q '^KINOSAIL_IMAGE=ghcr.io/mikeo7/kinosail-subtitles@sha256:a\{64\}$' "$fixture/app/.env"
grep -Fq 'cosign verify --certificate-identity-regexp ^https://github\.com/MikeO7/kinosail/\.github/workflows/subtitles-release\.yml@refs/tags/subtitles-v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$ --certificate-oidc-issuer https://token.actions.githubusercontent.com' "$fixture/compose.log"
...
PATH="$fixture/bin:$PATH" "$fixture/app/scripts/install.sh" "$fixture/media" 9080 >/dev/null
grep -Fq 'cosign verify --certificate-identity https://github.com/MikeO7/kinosail/.github/workflows/subtitles-release.yml@refs/tags/subtitles-v1.2.3 --certificate-oidc-issuer https://token.actions.githubusercontent.com' "$fixture/compose.log"
sed -i.bak '/^KINOSAIL_VERSION=/d' "$fixture/app/.env"
rm "$fixture/app/.env.bak"
