...
mkdir -p "$tmp/seed/apps/subtitles/scripts"
cp "$publisher" "$tmp/seed/apps/subtitles/scripts/publish-main.sh"
printf 'base\n' >"$tmp/seed/apps/subtitles/file"
git -C "$tmp/seed" add .
...
printf 'safe\n' >>"$tmp/worker/apps/subtitles/file"
git -C "$tmp/worker" commit -am safe >/dev/null 2>&1
(cd "$tmp/worker" && ./apps/subtitles/scripts/publish-main.sh) >/dev/null 2>&1
test "$(git -C "$tmp/worker" rev-parse HEAD)" = "$(git --git-dir="$tmp/remote.git" rev-parse main)"

printf 'dirty\n' >>"$tmp/worker/apps/subtitles/file"
if (cd "$tmp/worker" && ./apps/subtitles/scripts/publish-main.sh) >/dev/null 2>&1; then
...
if (cd "$tmp/outscope" && ./apps/subtitles/scripts/publish-main.sh) >/dev/null 2>&1; then
