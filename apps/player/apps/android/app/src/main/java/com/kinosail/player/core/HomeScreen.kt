...
internal fun HomeScreen(viewer: Viewer, catalog: CatalogModel, tv: Boolean, browse: () -> Unit,
    ...
    Box(Modifier.fillMaxSize().background(if (dark) KinoColor.background else MaterialTheme.colorScheme.background)) {
            ...
            LazyColumn(Modifier.weight(1f), contentPadding = PaddingValues(bottom = 24.dp),
                ...
                if (featured != null) item {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(if (tv) 32.dp else 16.dp),
                        verticalAlignment = Alignment.CenterVertically) {
                        HomeArtwork(featured, catalog, if (tv) 140 else 120)
                        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                            Text(featured.title, style = if (tv) MaterialTheme.typography.headlineLarge
                ...
                if (state.continueWatching.isNotEmpty()) item {
                    HomeShelf("Continue Watching", state.continueWatching.take(12), catalog, tv, open)
                }
                if (state.recent.isNotEmpty()) item {
                    HomeShelf("Recently Added", state.recent.take(24), catalog, tv, open)
                }
...
private fun HomeShelf(title: String, items: List<CatalogItem>, catalog: CatalogModel, tv: Boolean,
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        LazyRow(horizontalArrangement = Arrangement.spacedBy(if (tv) 20.dp else 12.dp)) {
            items(items, key = CatalogItem::id) { item ->
                ...
                } else androidx.compose.material3.Card(onClick = { open(item) }, modifier = Modifier.width(144.dp)) {
                    Column {
                        HomeArtwork(item, catalog, 144)
