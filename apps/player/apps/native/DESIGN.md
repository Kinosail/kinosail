...
The Apple Watch opens on a compact Remote page with the current player, title, progress and playback controls. After the viewer opts in, a vertical swipe opens the Heart page; the Remote page also links to it. Electric green marks play and the movie-time heart graph. Heart tracking begins only when the viewer taps Start heart graph and grants Health access; missing readings or playback positions appear as gaps. The watch reads Health locally and does not send heart data to the iPhone, Apple TV or Server.
