...
Home displays For you and My List above the feature, then up to four compact Continue watching or Continue listening rows. On iOS, Show more reveals the remaining loaded rows and Show less returns to four; Apple TV keeps four rows. Watch separates recently added movies and TV shows, then offers unwatched rows and up to four movie genre shelves from the loaded catalog. Listen separates recently added music and audiobooks. Apple TV can switch between these Home modes. Each continuation row combines a contained landscape backdrop or 2:3 poster fallback, title, metadata, true progress and play affordance; See all opens History. `ResumeRows` adapts from 420pt minimum columns on iOS and 640pt on tvOS. At accessibility text sizes, rows use one column, omit decorative thumbnails and let titles wrap without a line limit.
