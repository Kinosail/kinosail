...
iOS Home labels a saved Watch feature Watching and places it above Continue watching. Continue watching shows up to 15 landscape cards with direct resume, metadata and true progress. Without a saved title, the recent feature is labeled For you. Listen mode labels its saved feature Listening and keeps compact Continue listening rows with iOS expansion controls. Apple TV Home shows up to 15 saved titles in one Continue watching rail, including the first title, followed by Browse tiles for Movies, TV Shows, Music, Audiobooks, Photos and Collections. Browse uses raised dark cards with distinct solid off-white and Electric green geometric category marks and visible labels inside each focusable card. A final tile with the Kinosail mark switches Watch and Listen Home modes. Watch then separates recently added movies and TV shows, adds unwatched shelves and up to four movie genre shelves from the loaded catalog. Listen separates recently added music and audiobooks. At accessibility text sizes, card titles wrap and the shelf remains horizontally scrollable.
...
`TouchPlaybackView` owns the immersive iPhone/iPad controls: one dismissal/title row, play and ten-second seeking, a native slider with elapsed and remaining time, and direct audio/subtitle, speed, volume and Picture in Picture actions. The played timeline track uses Electric green; the remaining track, thumb and controls stay neutral white over black, with safe-area placement, semantic type and at least 44pt targets. Compact layouts keep the audio/subtitle action as a labeled accessibility icon. Controls fade together after four seconds of uninterrupted playback, stay available with VoiceOver or Switch Control, and retain keyboard shortcuts while hidden. Reduce Motion removes the fade.

The Apple Watch opens on a compact Remote page with the current player, title, progress and playback controls. After the viewer opts in, a vertical swipe opens the Heart page; the Remote page also links to it. Electric green marks play and the movie-time heart graph, including the peak reading at its movie position. Heart tracking begins only when the viewer taps Start heart graph and grants Health access; missing readings or playback positions appear as gaps. The watch reads Health locally and does not send heart data to the iPhone, Apple TV or Server.
