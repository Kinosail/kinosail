            ...
            if let failure { RetryState(message: failure) { loadRevision += 1 } }
                ...
                .safeAreaInset(edge: .bottom, spacing: 0) {
                    VStack(spacing: 10) {
                        ...
                        HStack {
                            Button("Previous", systemImage: "chevron.left") { turn(to: position.page - 1) }.labelStyle(.iconOnly).buttonStyle(.bordered).buttonBorderShape(.capsule).tint(KinoTheme.raised).foregroundStyle(KinoTheme.text).controlSize(.large).disabled(position.page <= 1)
                            Spacer()
                            ...
                            Spacer()
                            Button("Next", systemImage: "chevron.right") { turn(to: position.page + 1) }.labelStyle(.iconOnly).buttonStyle(.bordered).buttonBorderShape(.capsule).tint(KinoTheme.raised).foregroundStyle(KinoTheme.text).controlSize(.large).disabled(position.page >= position.total)
                        }
