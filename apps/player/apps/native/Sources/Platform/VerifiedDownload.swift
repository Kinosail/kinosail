  ...
  func decodeOffline<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
    ...
    if type == OfflineManifest.self {
      ...
      optional = ["retryAt", "retries", "startedAt"]
    } else {
      ...
      let _: OfflineManifest = try decodeOffline(OfflineManifest.self, from: JSONSerialization.data(withJSONObject: manifest))
    }
  ...
  struct VerifiedDownload: Codable, Sendable {
    let scope: String
    let key: String
    let uri: String
    ...
    static func failure(_ message: String) -> ClientError { .invalidInput(message) }

    func validate() throws {
      try manifest.validate()
      _ = try OfflineResource.url(uri)
      if let retryAt { guard retryAt.timeIntervalSince1970.isFinite, retryAt.timeIntervalSinceNow <= 3660 else { throw ClientError.invalidResponse } }
      guard OfflineManifest.digest(scope), OfflineManifest.digest(key), uri.utf8.count <= 2048,
            let url = URL(string: uri), let host = url.host,
            (url.scheme == "https" || url.scheme == "http"), !host.isEmpty,
  ...
  struct DownloadPreparation: Codable, Sendable {
    let scope: String
    let key: String
    let uri: String
    let kind: String
    ...
    func validate() throws {
      _ = try OfflineResource.url(uri)
      guard (0...5).contains(retries ?? 0) else { throw ClientError.invalidResponse }
      if let startedAt { guard startedAt.timeIntervalSince1970.isFinite, startedAt.timeIntervalSince1970 >= 0, startedAt.timeIntervalSinceNow <= 60 else { throw ClientError.invalidResponse } }
      if let retryAt { guard retryAt.timeIntervalSince1970.isFinite, retryAt.timeIntervalSinceNow <= 3660 else { throw ClientError.invalidResponse } }
      guard OfflineManifest.digest(scope), OfflineManifest.digest(key), uri.utf8.count <= 2048,
