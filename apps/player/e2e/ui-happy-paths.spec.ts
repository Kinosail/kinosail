...
test("Owner can create and revoke an API key and manage a Viewer Profile", async ({ page }, testInfo) => {
	...
	const key = page.getByLabel("Generated API key");
	await expect(key).toHaveValue(/^ks_/);
	await page.evaluate(() => Object.defineProperty(navigator, "clipboard", { configurable: true, value: { writeText: async (value: string) => { (window as Window & { copiedAPIKey?: string }).copiedAPIKey = value; } } }));
	await page.getByRole("button", { name: "Copy API key" }).click();
	await expect(page.locator(".copy-status")).toHaveText("Copied.");
	expect(await key.evaluate((input) => (window as Window & { copiedAPIKey?: string }).copiedAPIKey === (input as HTMLInputElement).value)).toBeTruthy();
	await page.evaluate(() => Object.defineProperty(navigator, "clipboard", { configurable: true, value: { writeText: async () => { throw new Error("denied"); } } }));
	await page.getByRole("button", { name: "Copy API key" }).click();
	await expect(page.locator(".copy-status")).toHaveText("Select Copy in your browser.");
	expect(await key.evaluate((input) => (input as HTMLInputElement).selectionStart === 0 && (input as HTMLInputElement).selectionEnd === (input as HTMLInputElement).value.length)).toBeTruthy();
...
test("Owner can share Library Content, open the claim, and revoke it", async ({ browser, page }, testInfo) => {
	...
	await authenticatedClaim.goto(new URL(claimURL!, page.url()).toString(), { waitUntil: "commit" });
...
test("Owner can authorize a waiting device with Quick Connect", async ({ page, request }) => {
	...
	const code = page.locator("[data-quick-connect-digit]");
	await expect(code).toHaveCount(6);
	await expect(code.first()).toHaveAttribute("maxlength", "1");
	await expect(code.first()).toHaveAttribute("inputmode", "numeric");
	await code.first().pressSequentially(pending.code);
	await expect(page).toHaveURL("/");
...
test("Owner can paste a Quick Connect code into the digit fields", async ({ page, request }) => {
	await login(page);
	const started = await request.post("/api/v1/quick-connect", { data: { device: "E2E pasted code" } });
	expect(started.status()).toBe(201);
	const pending = await started.json() as { code: string; secret: string };
	await page.goto("/quick-connect");
	await page.setViewportSize({ width: 390, height: 844 });
	...
	await Promise.all([
		page.waitForURL("/"),
		first.evaluate((input, code) => {
			const clipboardData = new DataTransfer();
			clipboardData.setData("text/plain", code as string);
			input.dispatchEvent(Object.defineProperty(new ClipboardEvent("paste", { bubbles: true }), "clipboardData", { value: clipboardData }));
		}, pending.code),
	]);
	const completed = await request.post("/api/v1/quick-connect/token", { data: { secret: pending.secret } });
	expect(completed.status()).toBe(201);
