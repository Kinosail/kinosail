...
Embedded and matching `.srt` or `.vtt` sidecar subtitles can appear in the player. If a subtitle needs conversion or the browser cannot display it, use **Compatibility**. Add a missing track beside the media file with a language code, such as `Movie.en.vtt`.
