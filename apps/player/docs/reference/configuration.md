...
Kinosail reads the file named by `KINOSAIL_CONFIG_FILE`. If that variable is empty, the Server can use `kinosail.yaml` below its data directory. The example file is [`kinosail.example.yaml`](https://github.com/Kinosail/kinosail/blob/main/apps/player/kinosail.example.yaml). Copy it to `kinosail.yaml` and set only the values that you own.
