...
- [server.go](../../internal/server/server.go) contains an always-visible search field that updates the main results after 250 ms;
- [browse.go](../../internal/server/browse.go) centralizes strict web/API query parsing, filtering, title/newest/year sorting, offsets, and 100-item default pages;
- [pwa.js](../../internal/server/static/pwa.js) progressively fetches the next page while retaining a real **Load more** link fallback; and
- [performance_benchmark_test.go](../../internal/server/performance_benchmark_test.go) already exercises a 10,000-item library.

The smallest coherent capability slice would be:

1. Extend the shared browse operation and `GET /api/v1/library` with one strictly validated, locale-aware bucket key; return available bucket labels/counts and active bucket metadata. The web adapter must call that same operation, consistent with [ADR 0005](../adr/0005-expose-complete-api-from-one-server-container.md).
