...
func projectShowPage(request *http.Request, progress *progressStore, show library.Show) showPageData {
	data := showPageData{Show: show, EpisodeCount: len(show.Episodes), BackdropID: showBackdropID(show), People: catalogapi.ShowCast(show)}
	for _, episode := range show.Episodes {
