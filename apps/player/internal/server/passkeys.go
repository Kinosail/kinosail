...
func newPasskeyAuth(rawURL string, profiles *profileStore) *passkeyAuth {
	...
	if err == nil {
		auth.flow, auth.err = sharedpasskeys.NewProfileFlow(sharedpasskeys.ProfileFlowConfig[viewerProfile]{
			Engine: engine, Current: currentViewer,
			Identity: func(profile viewerProfile) (string, string, []webauthn.Credential) {
				return profile.ID, profile.Name, profile.Passkeys
			},
			Owner: func(profile viewerProfile) bool { return profile.Owner }, Remote: func(profile viewerProfile) bool { return profile.Remote },
			...
			SignIn: func(writer http.ResponseWriter, request *http.Request, id string, public bool) error {
				if public {
					return profiles.signInStrongPublic(writer, request, id)
				}
				return profiles.signInStrong(writer, request, id)
			},
			AfterLogin: func(request *http.Request, profile viewerProfile, credential *webauthn.Credential) {
			...
			OnboardingNext: func(profile viewerProfile) bool {
...
func (auth *passkeyAuth) register(mux *http.ServeMux, allowLogin func(string) bool) {
	...
	mux.HandleFunc("POST /api/v1/passkeys/login/begin", func(writer http.ResponseWriter, request *http.Request) {
		if auth.engine.MatchesRequest(request, secureRequest(request)) && !allowLogin(request.RemoteAddr) {
