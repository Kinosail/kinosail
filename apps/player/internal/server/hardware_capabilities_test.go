...
`, arguments, servertest.HardwareSmokeHLS()+servertest.PlayableHLSWithFormat(640, 360, "hevc", "aac"))
...
`)
	handler := server.New(server.Config{FFprobe: probe, MediaDir: media, DataDir: data, CacheDir: cache, FFmpeg: ffmpeg, ProbeHardware: true, HardwareDevices: []string{device}, HardwareOS: "linux", HardwareArch: "amd64", RequireAuth: true})
	...
	for deadline := time.Now().Add(2 * time.Second); time.Now().Before(deadline); time.Sleep(10 * time.Millisecond) {
		...
		if strings.Contains(string(master), "1080p/index.m3u8") {
