...
func (auth *authentication) protect(next http.Handler, pattern func(*http.Request) string) http.Handler { //nolint:cyclop,gocognit // Authentication decisions remain linear and below the repository complexity ceiling.
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if !auth.pageOriginAllowed(writer, request) {
			return
		}
		...
		public, valid := auth.jellyfinMediaRoutePublic(writer, request, matched, matched == "" || auth.public(request, matched))
		if !valid {
			return
		}
		if owneraccess.ProfileID(request) != "" && !identitycore.ManagementBootstrapRoute(matched) {
			public = false
		}
		if public {
...
func (auth *authentication) pageOriginAllowed(writer http.ResponseWriter, request *http.Request) bool {
	if publicInternetRequest(request) || !navigation.IsBrowserRequest(request, authenticationErrorRequest(request)) {
		return true
	}
	return auth.passkeys.engine.RequirePageOrigin(writer, request, auth.passkeys.redirectPages && auth.passkeys.err == nil, secureRequest(request))
}
...
func (auth *authentication) serveIdentity(next http.Handler, writer http.ResponseWriter, request *http.Request, matched string) bool {
	...
	if access := auth.viewerAccess(profile, request, matched); access != identitycore.Allowed {
...
func (auth *authentication) setRequireMFA(ctx context.Context, required bool) (bool, error) {
	...
	revoke := required && !settings.RequireMFA
	settings.RequireMFA = required
	if !revoke {
		if err := auth.settings.save(settings); err != nil {
			return false, err
		}
	...
	auth.settings.value, auth.profiles.sessions = settings, sessions
	return true, nil
}
...
func (auth *authentication) public(request *http.Request, pattern string) bool {
	...
	return publicCapability(request, pattern)
}

func publicCapability(request *http.Request, pattern string) bool {
	if pattern == "GET /Videos/{id}/{stream...}" {
		return publicVideoStreamCapability(request)
	}
	if strings.HasPrefix(pattern, "GET /Videos/") || pattern == "GET /Audio/{id}/{stream}" {
		...
		return jellyfinPlaySessionQuery(request) != ""
	}
	return true
}

func publicVideoStreamCapability(request *http.Request) bool {
	path := strings.TrimPrefix(request.URL.Path, "/Videos/")
	slash := strings.IndexByte(path, '/')
	if slash < 0 {
		return false
	}
	if directJellyfinStreamPath(path) {
		return validPlaybackSession(jellyfinPlaySessionQuery(request))
	}
	stream := strings.ToLower(path[slash+1:])
	if _, sourceHLS := jellyfinSourceHLSFile(stream); hlsFile(stream) || sourceHLS {
		return validPlaybackSession(jellyfinPlaySessionQuery(request))
	}
	_, file, planned := plannedHLSFile(stream)
	return planned && hlsFile(file) && validPlaybackSession(jellyfinPlaySessionQuery(request))
}

func directJellyfinStreamPath(path string) bool {
	return strings.HasPrefix(path[strings.LastIndexByte(path, '/')+1:], "stream")
}
