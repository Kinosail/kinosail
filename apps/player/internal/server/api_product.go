...
type apiPlayback struct {
	...
	CompatibleDuration      float64           `json:"compatibleDuration,omitempty"`
	CompatibleProgressToken string            `json:"compatibleProgressToken,omitempty"`
...
func registerProductAPI(mux *http.ServeMux, api apiServices) {
	...
	mux.HandleFunc("GET /api/v1/me", productapi.Me(func(request *http.Request) productapi.MeState {
		return productapi.MeState{Server: api.settings.serverName(), ServerID: api.settings.jellyfinID(), Viewer: currentViewer(request), SSO: api.auth.sso, Language: preferredLanguage(request), LanguagePreference: languagePreference(request), Languages: languageOptions()}
	}))
...
func requestedPlaybackCapabilities(request *http.Request, settings *settingsStore) (ClientCapabilities, error) {
	hdrFormats, audioChannels, err := sharedplayback.RequestedDisplayCapabilities(request)
	if err != nil {
	...
	client := browserPlaybackCapabilitiesForRequest(request, settings, videoCodecs)
	if audioCodecs != nil {
