...
func webRevokeAgentConnection(connections *mcpConnections) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		if !httpguard.FormEncoded(request) || request.URL.RawQuery != "" || request.ParseForm() != nil || !httpguard.OnlyFormKeys(request.PostForm, "id") {
			localizedError(writer, request, "invalid connection", http.StatusBadRequest)
			return
		}
		id, ok := httpguard.RequiredValue(request.PostForm, "id", 128)
		if !ok {
			localizedError(writer, request, "invalid connection", http.StatusBadRequest)
			return
		}
