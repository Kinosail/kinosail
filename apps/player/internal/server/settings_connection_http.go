...
func onboardingConnectionTemplate() string {
	content := strings.NewReplacer(
		...
		`<h1>Make every device feel at home.</h1><p>Choose the address your phones, TVs, and browsers will trust. Kinosail stays on your network unless you separately enable remote access.</p>`, `<h1>Choose how devices connect.</h1><p>Secure local access stays on by default. Jellyfin apps require trusted HTTPS because many clients reject private certificates.</p>`,
		`<section class="wide"><h2>Trusted HTTPS with DuckDNS</h2>`, secureLocalOnboardingHTML+`<section class="wide" id="trusted-https"><h2>Trusted HTTPS with DuckDNS (Required for Jellyfin apps)</h2>`,
		...
		`<section><h2>Remote access comes later</h2>`, homeAssistantOnboardingHTML+`<section><h2>Remote access comes later</h2>`,
