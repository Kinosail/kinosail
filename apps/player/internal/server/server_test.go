...
func TestViewerCanOpenPlayer(t *testing.T) {
	...
	match := regexp.MustCompile(`/(?:watch|item)/([a-f0-9]+)`).FindStringSubmatch(homeResponse.Body.String())
	if len(match) != 2 {
...
func TestAdaptivePlayerUsesInBandStartupMeasurementAndSeamlessQualityControls(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"scheduleNetworkRetry(", "Auto · recovered", `preference === "auto" ? "Auto" : preference`,
	} {
