...
func TestApplicationShellUsesCurrentCombinedStylesheet(t *testing.T) {
	...
	if !bytes.Contains(page, []byte(`/static/app.css?v=electric-22`)) || bytes.Contains(page, []byte(`/static/app.css?v=impeccable-1`)) {
		t.Fatalf("application shell stylesheet = %q", page)
	}
	current := applicationShellCSSVersion([]byte(`<link rel="stylesheet" href="/static/app.css?v=electric-22">`))
	if !bytes.Contains(current, []byte(`/static/app.css?v=electric-22`)) {
...
func TestApplicationShellRefreshesCachedStylesheetVersions(t *testing.T) {
	for _, version := range []string{"72", "80", "81", "82", "83", "84", "85", "91", "93", "94", "impeccable-1", "electric-1", "electric-4", "electric-18"} {
		...
		want := []byte(`<link rel="stylesheet" href="/static/app.css?v=electric-22">`)
