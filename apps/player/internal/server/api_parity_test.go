...
func TestMetadataProviderAPIFailureIsExplicit(t *testing.T) {
	t.Parallel()
	...
	assertAPICalls(t, handler, token, []apiTestCall{{Method: http.MethodPost, Path: "/api/v1/items/" + id + "/metadata/refresh", Body: nil, Status: http.StatusBadGateway}})
}
...
func TestVersionedAPICoversBrowserOnlyCapabilities(t *testing.T) {
	...
	paths := []string{`/api/v1/passkeys/register/begin`, `/api/v1/session/oidc`, `/api/v1/watch-rooms/{id}/events`, `/api/v1/items/{id}/markers`, `/api/v1/items/{id}/playback-events`, `/api/v1/metadata/bulk`, `/api/v1/configuration/{key}`, `/api/v1/remote-access/wireguard`, `/api/v1/settings/trusted-https/validate`, `/api/v1/settings/trusted-https/test`, `/api/v1/backups/verify`, `/api/v1/viewing-imports/preview`, `/api/v1/viewing-syncs/{id}/run`, `/api/v1/supporter/activate`, `/api/v1/supporter/certificate.svg`, `/api/v1/supporter/certificates/patron-order.svg`, `/api/v1/supporter/certificates/living-standard.svg`}
	assertAPIBody(t, openapi, http.StatusOK, paths...)
	assertAPIBody(t, openapi, http.StatusOK, `"recognitionName"`, `"maxLength": 80`)
