...
func (auth *authentication) login(writer http.ResponseWriter, request *http.Request) { //nolint:cyclop,gocognit // Password, MFA, and session decisions remain one auditable login flow.
	...
	if publicInternetRequest(request) {
		localizedError(writer, request, "public password login is disabled; use a passkey or Quick Connect", http.StatusForbidden)
		return
	}
...
func (auth *authentication) setup(writer http.ResponseWriter, request *http.Request) {
	...
	profile, enrollment, err := auth.createFirstOwner(request.Context(), request.FormValue("name"), request.FormValue("password"), request.FormValue("totp") == "true", automaticUpdates)
	if err != nil {
