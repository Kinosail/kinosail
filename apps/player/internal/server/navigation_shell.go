...
func injectApplicationShell(page, navigation []byte) []byte {
	...
	if !bytes.Contains(page, []byte(`/static/supporter.js?v=`)) {
		page = bytes.Replace(page, []byte("</body>"), []byte(`<script defer src="/static/supporter.js?v=12"></script></body>`), 1)
	}
...
func applicationShellCSSVersion(page []byte) []byte {
	if start := bytes.Index(page, []byte(`/static/app.css?v=`)); start >= 0 {
		...
		if end := bytes.IndexByte(page[version:], '"'); end >= 0 {
			if bytes.Equal(page[version:version+end], []byte("electric-17")) {
				return page
			}
			page = append(append(append([]byte(nil), page[:version]...), []byte("electric-17")...), page[version+end:]...)
		}
	}
	return page
}
