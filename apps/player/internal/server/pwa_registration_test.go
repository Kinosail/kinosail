...
func assertOfflineShell(t *testing.T, worker *httptest.ResponseRecorder) {
	t.Helper()
	if !strings.Contains(worker.Body.String(), `const cacheName = "kinosail-shell-v47"`) || !strings.Contains(worker.Body.String(), `event.respondWith(navigation.catch(() => caches.open(cacheName).then((cache) => cache.match("/offline"))))`) || !strings.Contains(worker.Body.String(), `response.ok ? refreshOffline()`) {
...
func TestNavigationPagesRefreshCachedAssets(t *testing.T) {
	...
	for _, path := range []string{"/", "/settings"} {
		...
		handler.ServeHTTP(response, httptest.NewRequestWithContext(t.Context(), http.MethodGet, path, nil))
		body := response.Body.String()
		if response.Code != http.StatusOK || !strings.Contains(body, `main.kinosail.bundle.js?v=26`) || !strings.Contains(body, `app.css?v=electric-17`) {
