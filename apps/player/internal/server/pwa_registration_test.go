...
func TestPWAHandlesServiceWorkerRegistrationFailure(t *testing.T) {
	...
	if !strings.Contains(script, `serviceWorker.register("/service-worker.js?v=46").then`) || !strings.Contains(script, `.catch(() => {});`) {
...
func assertOfflineShell(t *testing.T, worker *httptest.ResponseRecorder) {
	t.Helper()
	if !strings.Contains(worker.Body.String(), `const cacheName = "kinosail-shell-v46"`) || !strings.Contains(worker.Body.String(), `event.respondWith(navigation.catch(() => caches.open(cacheName).then((cache) => cache.match("/offline"))))`) || !strings.Contains(worker.Body.String(), `response.ok ? refreshOffline()`) {
...
func TestOfflinePagesUseTheCurrentNavigationBundle(t *testing.T) {
	...
	for _, path := range []string{"/offline-downloads", "/offline"} {
		...
		if !strings.Contains(body, `downloads.js?v=24`) || strings.Contains(body, `main.kinosail.bundle.js?v=12`) || strings.Contains(body, `main.kinosail.bundle.js?v=16`) {
			t.Fatalf("%s can register an obsolete offline worker: %s", path, body)
		}
		if path == "/offline-downloads" && !strings.Contains(body, `main.kinosail.bundle.js?v=26`) {
...
func TestNavigationPagesRefreshCachedAssets(t *testing.T) {
	...
	for _, path := range []string{"/", "/settings"} {
		...
		handler.ServeHTTP(response, httptest.NewRequestWithContext(t.Context(), http.MethodGet, path, nil))
		body := response.Body.String()
		if response.Code != http.StatusOK || !strings.Contains(body, `main.kinosail.bundle.js?v=26`) || !strings.Contains(body, `app.css?v=electric-14`) {
