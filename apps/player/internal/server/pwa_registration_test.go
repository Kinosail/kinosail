...
func assertOfflineShell(t *testing.T, worker *httptest.ResponseRecorder) {
	t.Helper()
	if !strings.Contains(worker.Body.String(), `const cacheName = "kinosail-shell-v45"`) || !strings.Contains(worker.Body.String(), `event.respondWith(navigation.catch(() => caches.open(cacheName).then((cache) => cache.match("/offline"))))`) || !strings.Contains(worker.Body.String(), `response.ok ? refreshOffline()`) {
