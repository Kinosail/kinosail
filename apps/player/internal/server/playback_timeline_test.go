...
func TestJellyfinPlannedHLSFileAcceptsValidatedClientBitrate(t *testing.T) {
	recipe, file, planned := plannedHLSFile("p/t-a0-s0-none-t0-b356617565-k3kuzn_3vmv4/index.m3u8")
	if !planned || file != "index.m3u8" || recipe.maxBitrate != 356_617_565 || len(recipe.omitted) != 1 {
		t.Fatalf("planned Jellyfin HLS file = %#v, %q, %t", recipe, file, planned)
	}
	if _, err := playback.ParseHLSRecipe("t-a0-s0-none-t0-b1000000000001", hlsPolicy()); err == nil {
		t.Fatal("playback recipe accepted bitrate above the Jellyfin request limit")
	}
}
