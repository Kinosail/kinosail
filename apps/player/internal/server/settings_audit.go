...
func (store *settingsStore) auditSnapshot(request *http.Request) map[string]string { //nolint:cyclop // Each mutable settings projection is intentionally allowlisted.
	...
	case "/settings/subtitles", "/api/v1/settings/subtitles":
		return map[string]string{"language": store.value.SubtitleLanguage}
	case "/settings/scans", "/api/v1/settings/scans":
	...
	case "/settings/libraries", "/settings/libraries/remove", "/api/v1/libraries":
		return map[string]string{"libraries": strings.Join(store.value.Libraries, ",")}
	case "/settings/configuration", "/settings/configuration/reset", "/onboarding/tmdb":
...
func configurationAuditValue(store *settingsStore, key string) map[string]string {
	if key == tmdbConfigurationKey {
		field := store.config.Public(tmdbTokenKey)
		return map[string]string{"key": key, "source": string(field.Source), "configured": strconv.FormatBool(field.Configured)}
	}
