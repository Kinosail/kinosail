...
func TestExternalConfigurationOverridesAndLocksSettings(t *testing.T) { //nolint:cyclop,funlen // One scenario checks API and web rendering without weakening the secret boundary.
	...
	for _, expected := range []string{
		...
		`href="/settings/configuration#integrations.tmdb"`, `href="/settings/configuration#integrations.oidc"`,
	...
	if configurationPage.Code != http.StatusOK || strings.Contains(configurationHTML, "hidden-token") || !strings.Contains(configurationHTML, "Movie artwork and details") || !strings.Contains(configurationHTML, "Configured via Docker: <code>KINOSAIL_TMDB_TOKEN</code>.") || !strings.Contains(configurationHTML, "Configured via Docker: <code>KINOSAIL_SCIM_TOKEN</code>.") || !strings.Contains(configurationHTML, "Configuration key: <code>integrations.tmdb.token</code>") {
