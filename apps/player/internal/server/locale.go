...
func loadLocalization() (*i18n.Bundle, []catalogMessage) {
	...
	data, err := localeCatalogs.ReadFile("locales/active.en.json")
	if err != nil {
		panic(err)
	}
...
func newLocalizedTemplate(name, source string) *localization.TemplateSet {
	source = strings.ReplaceAll(source, `/static/main.kinosail.bundle.js?v=12`, `/static/main.kinosail.bundle.js?v=29`)
	source = strings.Replace(source, `<html lang="en">`, `<html lang="en" data-theme="dark">`, 1)
	return localization.NewTemplateSet(name, string(applicationShellCSSVersion([]byte(source))), "29", localeCatalog, supportedLanguages, httpguard.CSRFTemplateSource, httpguard.CSRFParseFuncs(uiIcon), localeTemplateRuntime)
}
...
func localizedError(writer http.ResponseWriter, request *http.Request, message string, status int) {
	...
	if request.URL.Path == "/setup" || strings.HasPrefix(request.URL.Path, "/onboarding/") {
		writer.Header().Set("Content-Type", "text/html; charset=utf-8")
		writer.WriteHeader(status)
		returnPath := request.URL.Path
		if returnPath == "" {
			returnPath = "/setup"
		}
		if err := onboardingErrorView.Execute(writer, request, struct {
			Message, ReturnPath string
		}{message, returnPath}); err != nil {
			return
		}
		return
	}
	writeWebError(writer, request, message, status)
}
