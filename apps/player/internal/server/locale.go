...
func newLocalizedTemplate(name, source string) *localization.TemplateSet {
	source = strings.ReplaceAll(source, `/static/main.kinosail.bundle.js?v=12`, `/static/main.kinosail.bundle.js?v=25`)
	source = strings.Replace(source, `<html lang="en">`, `<html lang="en" data-theme="dark">`, 1)
	return localization.NewTemplateSet(name, string(applicationShellCSSVersion([]byte(source))), "25", localeCatalog, supportedLanguages, csrfTemplateSource, csrfParseFuncs(), localeTemplateRuntime)
}
