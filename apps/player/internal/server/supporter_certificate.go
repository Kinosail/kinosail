...
func supporterCertificateArt(family string, rank int) template.HTML {
	if (family != patronOrderFamily && family != livingStandardFamily) || rank < 1 || rank > 10 {
		return ""
	}
	...
	return template.HTML(content[strings.Index(content, ">")+1 : strings.LastIndex(content, "</svg>")]) //nolint:gosec // Fixed embedded artwork; no user content.
}
...
func (program *supporterProgram) writeCurrentCertificate(writer http.ResponseWriter) {
	status := program.status()
	family := patronOrderFamily
	if status.LivingStandard != nil && status.LivingStandard.Active || status.PatronOrder == nil && status.LivingStandard != nil {
		family = livingStandardFamily
	}
