...
func (broker *quickConnectBroker) application(profiles *profileStore) *quickconnect.Application {
	...
	return quickconnect.NewApplication(broker.connections, quickconnect.Limiters{Requests: &broker.requests, Starts: &broker.starts, Polls: &broker.polls}, adapter)
}
...
func (broker *quickConnectBroker) register(mux *http.ServeMux, profiles *profileStore) {
	...
	mux.HandleFunc("POST /api/v1/quick-connect/cancel", broker.application(nil).CancelAPI)
	mux.HandleFunc("POST /api/v1/quick-connect/{code}", broker.approveAPI(profiles))
	mux.HandleFunc("GET /api/v1/quick-connect/pending", broker.application(profiles).PendingTVsAPI)
	mux.HandleFunc("GET /api/v1/quick-connect/{code}", broker.application(profiles).PreviewAPI)
	mux.HandleFunc("GET /connect", broker.connectPage)
