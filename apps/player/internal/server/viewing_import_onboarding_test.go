...
func TestOwnerCanStartViewingMigrationFromOnboarding(t *testing.T) { //nolint:cyclop,funlen // One onboarding scenario covers preview-safe migration entry.
	...
	assertAPIBody(t, connection, http.StatusOK, "Connect the devices you already own.", "Ready on this Server", "Secure local access", "On by default.", "Jellyfin apps", "Trusted HTTPS for phones, TVs, and Jellyfin apps", "Required for Jellyfin apps. Recommended for phones and TVs.", "Jellyfin routes stay unavailable.", "Remote access comes later", `aria-current="step"`, `aria-label="Account complete"`, `id="trusted-https-configuration"`, `href="#trusted-https-configuration"`, `action="/onboarding/jellyfin"`, `action="/onboarding/trusted-https"`, `href="/onboarding/household"`)
