...
const appHeader = `<header class="app-header"><a class="brand-lockup" href="/"><img class="brand-icon" src="/static/icon.svg?v=8" alt=""><h1>{{.ServerName}}</h1><span class="web-beta-badge">Beta</span></a><form class="search" action="/" role="search"><label for="library-search">{{t "Search library"}}</label><input id="library-search" type="search" enterkeyhint="search" autocomplete="off" autocapitalize="none" spellcheck="false" aria-label="{{t "Search all libraries"}}" name="q" value="{{.Query}}" placeholder="{{t "Search all libraries"}}" hx-get="/" hx-target="#main" hx-select="#main" hx-swap="outerHTML" hx-push-url="true" hx-trigger="input changed delay:250ms, search" aria-keyshortcuts="/"><input type="hidden" name="sort" value="{{.Sort}}"><input type="hidden" name="view" value="all"></form><a class="header-supporter mobile-supporter" href="{{if .Owner}}/supporter{{else}}/supporter/collection{{end}}">Support Kinosail</a><nav aria-label="Main navigation" class="mobile-navigation" data-mobile-tabs data-nav-profile="{{.ViewerID}}">{{range .NavigationPrimary}}<a {{if .Active}}class="active" aria-current="page"{{end}} href="{{.Href}}">{{.Name}}</a>{{end}}{{range .NavigationMore}}<a class="nav-main-overflow{{if .Active}} active{{end}}"{{if .Active}} aria-current="page"{{end}} href="{{.Href}}">{{.Name}}</a>{{end}}{{if .Owner}}<a class="nav-main-supporter" href="/supporter">Supporter</a><a class="nav-main-edit" href="/settings#navigation">Edit navigation</a>{{end}}` + appMobileMore + `</nav><aside class="desktop-sidebar"><nav aria-label="Main navigation">{{range sidebarGroups .NavigationPrimary .NavigationMore}}<section class="sidebar-group" aria-labelledby="sidebar-{{.ID}}"><h2 id="sidebar-{{.ID}}">{{t .Name}}</h2>{{range .Links}}<a{{if .Active}} class="active" aria-current="page"{{end}} href="{{.Href}}"><span>{{.Name}}</span></a>{{end}}</section>{{end}}{{if .Owner}}<a class="sidebar-edit" href="/settings#navigation">Edit navigation</a>{{end}}</nav></aside><div class="header-actions"><a class="header-supporter" href="{{if .Owner}}/supporter{{else}}/supporter/collection{{end}}">Support Kinosail</a><details class="header-compact-menu"><summary>{{t "Actions"}}</summary><div class="header-compact-panel">{{if .CommandMenu}}<button class="quiet command-open" type="button" data-command-open aria-keyshortcuts="Control+K Meta+K">{{t "Actions"}} <kbd data-command-shortcut>⌘K</kbd></button>{{else}}<a class="header-link command-open" href="/">{{t "Browse library"}}</a>{{end}}<button hidden class="quiet" type="button" data-install>{{t "Install"}}</button><div class="header-utility-links{{if .Owner}} owner-utilities{{end}}"><a class="header-link" href="/quick-connect">{{t "Quick Connect"}}</a>{{if .Owner}}<form action="/scan" method="post"><button class="quiet">{{t "Sync"}}</button></form><a class="header-link" href="/settings">{{t "Settings"}}{{if .UpdateAvailable}} · Update{{end}}</a>{{end}}</div>{{if .CanLogout}}<div class="header-account" role="group" aria-label="{{t "Profile"}}"><a class="header-link profile-chip" href="/account">{{.ViewerName}}</a><form action="/logout" method="post"><button class="quiet">{{t "Sign out"}}</button></form></div>{{end}}</div></details></div></header>`
...
{{$id := ""}}{{$title := ""}}{{$href := ""}}{{$meta := ""}}{{$hasArt := false}}{{$label := "View details"}}{{$details := ""}}{{$poster := ""}}{{$featuredResume := false}}
{{if .Continue}}{{$item := index .Continue 0}}{{$id = $item.ID}}{{$title = $item.Title}}{{$href = printf "/watch/%s" $item.ID}}{{$meta = $item.Resume}}{{$label = "Resume"}}{{$details = printf "/item/%s" $item.ID}}{{$poster = index .HomeArtwork $item.ID}}{{$hasArt = or (ne $item.Backdrop "") (ne $item.ShowBackdrop "") (ne (index .HomeArtwork $item.ID) "")}}
{{else if .Recent}}{{$item := index .Recent 0}}{{$id = $item.ArtworkID}}{{$title = $item.Title}}{{$href = $item.Href}}{{if $item.PlayHref}}{{$href = $item.PlayHref}}{{$details = $item.Href}}{{$label = "Play"}}{{end}}{{$meta = $item.Meta}}{{$poster = $item.ArtworkID}}{{$hasArt = ne $item.ArtworkID ""}}{{end}}
{{$featuredResume = and (eq $label "Resume") $hasArt (ne $title "")}}
{{if and $title $hasArt}}<section class="home-feature" data-artwork="{{if $poster}}poster{{else}}backdrop{{end}}" data-palette-id="{{$id}}" aria-label="Featured title"><img src="{{if $poster}}/art/{{$poster}}{{else}}/backdrop/{{$id}}{{end}}" alt="" width="{{if $poster}}400{{else}}1600{{end}}" height="{{if $poster}}600{{else}}900{{end}}" fetchpriority="high"><div class="home-feature-copy"><h2>{{$title}}</h2><div class="home-feature-status"><p data-feature-meta>{{$meta}}</p><div class="watch-progress" data-watch-progress="{{$id}}" hidden><span data-watch-remaining></span><progress max="100" value="0" aria-label="Watch progress"></progress></div></div><div class="home-feature-actions"><a class="button" data-feature-action href="{{$href}}">{{if $details}}{{icon "play"}}{{end}}<span data-feature-label>{{t $label}}</span></a>{{if $details}}<a class="button quiet" href="{{$details}}">{{t "View details"}}</a>{{end}}{{if $featuredResume}}<form action="/continue-watching/{{$id}}/remove" method="post"><button class="home-feature-remove" aria-label="{{t "Remove"}} {{$title}} {{t "from Continue Watching"}}">{{t "Remove"}}</button></form>{{end}}</div></div></section>{{end}}

{{if and .Continue (or (not $featuredResume) (gt (len .Continue) 1))}}<section class="home-shelf continue-shelf"><header><h2>Continue watching</h2><a href="/?view=history">See all</a></header><div class="grid rail resume-grid">{{range $position, $item := .Continue}}{{if and (or (not $featuredResume) (ne $item.ID $id)) (or (lt $position 4) (and $featuredResume (lt $position 5)))}}<article class="card resume-card"><a class="resume-link" href="/watch/{{$item.ID}}" data-feature-id="{{$item.ID}}" data-feature-title="{{$item.Title}}" data-feature-meta="{{$item.Resume}}" data-feature-label="{{t "Resume"}}" data-feature-art="{{index $.HomeArtwork $item.ID}}">{{with (index $.HomeArtwork $item.ID)}}<img class="poster" src="/art/{{.}}" alt="" width="400" height="600" decoding="async" {{if lt $position 2}}fetchpriority="high"{{else}}loading="lazy"{{end}}>{{else}}<div class="poster">{{icon "play"}}</div>{{end}}<h3>{{with (index $.HomeShowTitle $item.ID)}}{{.}} · {{end}}{{$item.Title}}</h3><small>{{$item.Resume}}</small><div class="watch-progress" data-watch-progress="{{$item.ID}}" hidden><span data-watch-remaining aria-hidden="true"></span><progress max="100" value="0" aria-label="Watch progress"></progress></div><span class="resume-action" aria-label="{{t "Resume"}}">{{icon "play"}}</span></a><form action="/continue-watching/{{$item.ID}}/remove" method="post"><button class="quiet" aria-label="{{t "Remove"}} {{$item.Title}} {{t "from Continue Watching"}}">Remove</button></form></article>{{end}}{{end}}</div></section>{{end}}
{{range .HomeShelves}}<section class="{{if .Genres}}movie-genres {{end}}home-shelf" data-home-shelf="{{.Key}}"><header><h2>{{.Title}}</h2></header>{{if .Genres}}{{range .Genres}}<div class="movie-genre"><h3>{{.Name}}</h3><div class="grid rail recent-grid">{{range .Items}}{{template "homeShelfCard" .}}{{end}}</div></div>{{end}}{{else}}<div class="grid rail recent-grid">{{range .Items}}{{template "homeShelfCard" .}}{{end}}</div>{{end}}</section>{{end}}
...
{{if .TMDB}}<footer><a href="https://www.themoviedb.org">TMDB</a> · This product uses the TMDB API but is not endorsed or certified by TMDB.</footer>{{end}}</main>{{languagePicker}}<script src="/static/supporter.js?v=17"></script></body></html>
...
func newApplication(config Config) http.Handler { //nolint:funlen,cyclop,gocognit // The composition root validates defaults and wires every adapter to shared application services.
	...
	registerSettings(mux, settings, updates, index, progress, auth, config.InternetAccess, config.TrustedHTTPS, quickConnect, shares, hls, metadata, markers, backups, maintenance, viewingImports, homeAssistant, events, rooms, config.AuthURL)
