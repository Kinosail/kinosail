...
{{if .MovieGenres}}<section class="movie-genres home-shelf"><header><h2>Movie genres</h2></header>{{range .MovieGenres}}<div class="movie-genre"><h3>{{.Name}}</h3><div class="grid rail recent-grid">{{range .Items}}{{template "homeShelfCard" .}}{{end}}</div></div>{{end}}</section>{{end}}
