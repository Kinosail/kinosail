...
make_case() {
  ...
  printf '%s\n' 'KINOSAIL_REMOTE_MODE=off' 'KINOSAIL_IMAGE=ghcr.io/kinosail/kinosail-player@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' >"$target/.env"
}
...
grep -Fqx 'KINOSAIL_IMAGE=localhost/kinosail:dev' "$source_case/.env"
