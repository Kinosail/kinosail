...
func (fixture HardwareTranscodeFixture[S]) HardwareCapabilitiesDriveAutomaticSelectionAndVisibleFallback(t *testing.T) { //nolint:cyclop,funlen // One end-to-end scenario proves selection, delivery, and diagnostics together.
	t.Parallel()
...
`, arguments, HardwareSmokeHLS()+PlayableHLSWithFormat(640, 360, "hevc", "aac"))
	WriteExecutable(t, ffmpeg, script)
	probe := filepath.Join(tools, "ffprobe")
	WriteExecutable(t, probe, `#!/bin/sh
printf '%s' '{"streams":[{"codec_type":"video","codec_name":"hevc","width":1920,"height":1080},{"codec_type":"audio","codec_name":"aac"}]}'
`)
	handler := fixture.New(HardwareTranscodeConfig[S]{FFprobe: probe, MediaDir: media, DataDir: data, CacheDir: cache, FFmpeg: ffmpeg, ProbeHardware: true, HardwareDevices: []string{device}, HardwareOS: "linux", HardwareArch: "amd64", RequireAuth: true})
	...
	AssertAPIBody(t, hardware, http.StatusOK, `"selected":"qsv"`, "Intel Quick Sync", `"supported":true`, `"usable":true`, `"status":"Smoke check passed"`, "NVIDIA NVENC/NVDEC", "Container Toolkit", `"supported":false`, "native macOS Server", "native Windows Server", "Linux V4L2 hardware encoder", "Windows Media Foundation")
	...
	request = httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/hls/"+id+"/p/t-a0-s0-none-t0-b0-chevc-z1920x1080/index.m3u8", nil)
	request.AddCookie(owner)
	handler.ServeHTTP(httptest.NewRecorder(), request)
	variantsReady := false
	for deadline := time.Now().Add(2 * time.Second); time.Now().Before(deadline); time.Sleep(10 * time.Millisecond) {
		master, _ := os.ReadFile(filepath.Join(cache, id+"-plan-t-a0-s0-none-t0-b0-chevc-z1920x1080", "index.m3u8"))
		if strings.Contains(string(master), "1080p/index.m3u8") && strings.Count(string(master), "#EXT-X-STREAM-INF:") == 1 {
...
func masterCodec(t *testing.T, cache, id string) string {
	t.Helper()
	master, err := os.ReadFile(filepath.Join(cache, id+"-plan-t-a0-s0-none-t0-b0-chevc-z1920x1080", "index.m3u8"))
	if err != nil {
		t.Fatal(err)
	}
