...
targets=("$@")
if (( ${#targets[@]} == 0 )); then
	targets=(player subtitles dashboard packages)
fi
...
for target in "${targets[@]}"; do
	...
	packages)
		...
		output="$(cd "$repo" && "$tools/deadcode" -filter="$module" ./apps/player/... ./apps/subtitles/... ./apps/dashboard/... ./packages/... | sed -E '\#^packages/(archivetest|commandtest|configurationtest|servertest)/#d')"
		test_output="$(cd "$repo" && "$tools/deadcode" -test -filter="^$module/(archivetest|commandtest|configurationtest|servertest)$" ./apps/player/... ./apps/subtitles/... ./apps/dashboard/... ./packages/...)"
