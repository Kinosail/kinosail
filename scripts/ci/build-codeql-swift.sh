#!/usr/bin/env bash
# Separate targets avoid accumulating two extractor builds on one runner.
set -euo pipefail
...
# Set the driver thread count explicitly: Xcode versions differ in translating
# the WMO settings. One thread still emits the per-file objects Xcode expects.
# IDE index output is unrelated to CodeQL's traced AST extraction.
...
xcodebuild -project "$repo/apps/player/apps/native/Kinosail.xcodeproj" \
  ...
  COMPILER_INDEX_STORE_ENABLE=NO 'OTHER_SWIFT_FLAGS=$(inherited) -num-threads 1' \
