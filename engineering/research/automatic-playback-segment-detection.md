...
The current store persists `Revision`, final markers, and owner-suppressed types, but it still cannot distinguish a model/config change from a content change or recalibrate without decoding again. [current marker store](../../packages/markers/store.go)
