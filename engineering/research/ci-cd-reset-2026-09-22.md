...
2. `app.yml` runs the complete selected Player, Subtitles, or Dashboard suite. Lint, race/coverage, Go vulnerability checks, production-container and browser checks, installer checks, and Player Swift compilation run as relevant. Its jobs read the plan directly, without a serial selection runner. The final app check validates the plan and fails closed on missing, cancelled, failed, or unexpected skips.
