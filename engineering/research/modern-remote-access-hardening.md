...
The current [`unsafeCrossOrigin`](../../apps/player/internal/server/security.go) fallback looks only for `kinosail_session` when `Origin` is absent. A direct-public browser uses `__Host-kinosail_session`, so a cookie-authenticated mutation with no `Origin` and no trustworthy `Sec-Fetch-Site` is not rejected by that branch. The same function compares only `origin.Host` to `request.Host`; it does not compare the complete scheme, host, and effective port.
...
[`allowedOutboundIP`](../../apps/player/internal/server/security.go) currently rejects unspecified, multicast, and link-local addresses but permits loopback and private addresses. The shared client is used by metadata, subtitle, artwork, and notification adapters. Some installation-owned integrations intentionally need a local destination; remote-service responses and public-provider DNS must not receive that privilege.
