...
- [`internal/server/security.go`](../../apps/player/internal/server/security.go) marks public requests through server-created context rather than a Viewer-supplied header, validates exact request origins, applies session CSRF, strict range parsing, request limits, IPv6-aware source limits, and local/public outbound-network policies.
- [`internal/server/auth_remote_routes.go`](../../apps/player/internal/server/auth_remote_routes.go) and [`internal/server/auth.go`](../../apps/player/internal/server/auth.go) apply a public route inventory, reject Owner and API-key sessions, require a distinct public session, and re-evaluate Viewer policy.
- [`internal/server/passkeys.go`](../../apps/player/internal/server/passkeys.go) permits public passkey login only for remote-enabled non-Owner Viewers; [`internal/server/auth_login.go`](../../apps/player/internal/server/auth_login.go) rejects public password login. Quick Connect is the compatibility path for devices that cannot perform WebAuthn.
- [`internal/server/sessions.go`](../../apps/player/internal/server/sessions.go) creates server-side opaque public sessions with an eight-hour absolute limit, a ten-session per-Viewer cap, strong-authentication state, and the current Profile revision.
- [`internal/server/media_shares.go`](../../apps/player/internal/server/media_shares.go) has selected-item, expiring, revocable, device-limited grants with hashed claim/session tokens. The browser claim secret is carried in the URL fragment and removed from browser history before it is submitted.
- [`internal/server/api_remote.go`](../../apps/player/internal/server/api_remote.go) makes the kill switch close the public listener and revoke public sessions, public passkey ceremonies, remote Quick Connect state, and all Media Shares.
...
1. **It is direct-only.** [`internal/server/api_remote.go`](../../apps/player/internal/server/api_remote.go) reports `directOnly: true`; there is no connector, reverse tunnel, or relay fallback. It therefore cannot solve CGNAT or blocked inbound traffic.
2. **Setup is not one-click.** [`scripts/setup-remote-access.sh`](../../apps/player/scripts/setup-remote-access.sh) sends the Owner to DuckDNS, asks them to create a name and copy a long-lived account token, rewrites environment configuration, requires a restart, and instructs them to forward public TCP 443. Each of those is a support and security failure point.
3. **“Ready” is not an external reachability proof.** [`internal/server/remote_readiness.go`](../../apps/player/internal/server/remote_readiness.go) checks local state, listener state, certificate lifetime, policy, and a Viewer passkey, but does not prove a fresh connection from outside the home network. A correct local listener and DDNS update can still be unreachable behind CGNAT, double NAT, or a firewall.
