...
Move playlist/collection creation and sort forms out of the home feed and into contextual browse tools. The current home mixes the main library with two creation forms and sort controls ([current template](../../apps/player/internal/server/server.go)); that makes the opening page serve incompatible jobs at once.
