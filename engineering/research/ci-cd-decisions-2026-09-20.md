...
Swift analysis is split into iOS and tvOS jobs with separate analysis categories. Each performs a clean, single-architecture whole-module build with one compiler thread and disabled compile caches, retaining both platform-specific code paths. Xcode `-jobs` controls build operations but does not constrain the legacy Swift driver: [Swift Build](https://github.com/swiftlang/swift-build/blob/main/Sources/SWBCore/SpecImplementations/Tools/SwiftCompiler.swift) derives its parallelism from CPU count. Whole-module mode avoids parallel copies of the same module AST. The driver receives an explicit `-num-threads 1` because hosted Xcode did not translate the parallel-WMO setting consistently. IDE index-store generation is disabled after a hosted filename-count error; CodeQL extracts the compiler AST independently of that index. The previous successful scan logged up to 5,561 MB of swap usage. Build and resource logs are retained on failure. Standard `macos-26-intel` runners provide 14 GB rather than the 7 GB ARM runners ([GitHub runner specifications](https://docs.github.com/en/actions/reference/runners/github-hosted-runners)). A 30-minute per-platform bound replaces one serial 60-minute scan; the limit is a failure bound, not a performance target. Splitting the work and bounding concurrency is a response to the observed stall; hosted runs must establish whether it resolves it. GitHub recommends [single-architecture standard Xcode builds for Swift analysis](https://docs.github.com/en/code-security/reference/code-scanning/codeql/build-options-for-compiled-languages#customizing-swift-compilation-in-a-codeql-analysis-workflow).
...
Validation in progress: 46 focused CI tests, the complete repository tooling suite, actionlint, changed-shell ShellCheck, workflow boundary validation, and the 300-line cap passed locally. Independent review found no blocking issue in the graph, permissions, gate propagation, release compatibility, or publication prerequisites. [PR #50](https://github.com/Kinosail/kinosail/pull/50) records hosted validation. All three app `verify-changed` commands passed; Dashboard included all 71 browser tests. Clean local iOS/tvOS builds passed with observed `-Onone -whole-module-optimization -num-threads 1` invocations. Local compilation does not prove CodeQL extraction or physical-device behavior. The initial unified hosted graph passed all four quality gates; the final Swift resource correction requires fresh hosted validation. Signed image publication and production promotion still need verification on this revision. Live deployment and physical devices are separate evidence boundaries.


### Critical-path follow-up

The first unified run measured Player production/browser verification at 10m48s and Dashboard runtime/browser verification at 7m52s. Browser projects ran serially. Player and Subtitles now build and scan once per selected architecture, transfer that exact image as an immutable run artifact, and exercise each selected browser on a separate runner with its own container and state. The full browser matrix remains Chromium/Firefox/WebKit; ARM64 runtime coverage remains a single independent shard. Dashboard likewise isolates its three browser projects, running container and ARM64 build checks once. Required aggregators include image preparation and the complete matrix result; missing images, failed transfers, cancelled shards, and any browser failure block merge.

CI Python test-only changes now select the tooling suite and Python security scan; a Swift scan-build script change selects tooling and Apple verification. Unknown CI production inputs and any mixed selector/workflow edit still select everything. These are explicit dependency boundaries, not reduced test coverage. New local regression checks cover the image producer/consumer relationship, matrix isolation, required preparation jobs, and scope combinations. Hosted timings must establish the resulting speedup; no percentage improvement is claimed before that comparison.
