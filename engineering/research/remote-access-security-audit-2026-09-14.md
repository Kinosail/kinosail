...
8. **Recovery scripts:** setup reruns preserve the original local authentication address, including the automatic/empty default. Switching from HTTPS to WireGuard restores it too. Disable selects the installed source or release Compose file and stops the old service before recreation, so a failed recreation cannot leave the previous public listener serving. It stops active WireGuard before restarting locally. The CLI does not claim to revoke persisted sessions: use the Settings kill switch for authorization revocation, and remove the router rule when stopping access.
...
- Both setup scripts: interactive fixtures for TCP-only port 443 guidance, token redaction, repeated setup, automatic/local origin restoration, HTTPS-to-WireGuard changes, source installs, and failure after stopping the old service. Host service commands are stubbed.
