...
- Public HTTP always uses the closed public route policy. Neither an Owner session nor an API key grants administration there. Direct internet requests to the LAN listener also receive that policy, based on the socket peer address; headers cannot establish locality. The legacy Viewer WireGuard subnet is treated as public policy traffic. A router/proxy that hides internet clients behind a private source address still needs correct deployment configuration.
