...
Player native authority lives under `apps/player/apps/native/assets/source/`: `icon.svg`, `tv-foreground.svg`, `tv-background.svg`, `tv-banner.svg`, `top-shelf.svg` and `top-shelf-wide.svg`. `apps/player/apps/native/scripts/generate-brand-assets.sh` rasterizes and resizes with `sips`, exports the iOS app icon as RGB through `ffmpeg`, and populates the active Swift iOS/tvOS asset catalogs. TV foreground/background layers stay separate and Apple owns final icon masking. The script also maintains the existing reference raster export locations; this does not make the legacy client an active build target.
