...
const entry = { title: 'Backups', description: 'Keep state safe', url: '/owner-guide/backups/', content: 'encrypted recovery archive', product: 'Player' };
const settle = () => new Promise(resolve => setImmediate(resolve));
function setup(fetch, mobile = false) {
  const dom = new JSDOM(`<button data-theme-toggle></button><button data-menu-toggle></button><aside data-sidebar><form data-search-form data-index-url="/search.json"><input data-search-input><div data-search-results hidden></div></form></aside><main><a href="/">Content</a></main>`, { url: 'https://kinosail.com/', runScripts: 'outside-only' });
  const { window } = dom;
...
test('full text search caches the index, uses literal text, and supports no results', async () => {
  let requests = 0;
  const ui = setup(async () => { requests++; return response([entry, { ...entry, title: '<img src=x onerror=alert(1)>', url: '/test/' }]); });
...
test('invalid indexes never create navigable results', async () => {
  const cases = [null, {}, [null], [{ ...entry, title: 4 }], [{ ...entry, description: undefined }], [{ ...entry, content: 'x'.repeat(100000) }], Array(1001).fill(entry), ...['javascript:alert(1)', '//evil.test/', '/\\evil.test'].map(url => [{ ...entry, url }])];
