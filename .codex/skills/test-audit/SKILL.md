---
name: test-audit
description: "Use when writing or reviewing Kinosail tests, or auditing duplicate, implementation-coupled, or low-value tests and their test-only production seams."
---

# Test Audit

Three modes share one value bar. Use the authoring gate for new or changed tests.
Use audit mode for a focused sweep of tests that duplicate stronger proof,
assert implementation details, or keep test-only production seams alive. Use
campaign mode only when asked to audit a whole subsystem; read
[CAMPAIGN.md](CAMPAIGN.md) first. Optimize for confidence, not deletion count.
Repository `AGENTS.md` policies, including required negative tests and delivery
rules, still apply.
...
For a bug fix, demonstrate that its regression test fails on pre-fix code for
the intended reason and passes after the owner-boundary repair. If that control
cannot be run, state why and do not claim red-green proof. One regression at
the owner boundary covers the bug; another layer needs a distinct failure mode.
...
Keep a test when it independently enforces a public API, protocol,
configuration, migration, storage, security, platform, default, generated
asset, package, release, or architecture contract. Also keep:
...
Follow the current `AGENTS.md` verification and delivery rules.

1. Run the smallest owner and sibling tests using the owning Go, Python, shell,
   native, or browser test command.
...
4. Read `scripts/ci/affected.py` to identify suites selected for the changed
   paths. Run `make max-loc` and the affected app's `make verify-changed`
   after committing, plus focused tooling checks when tooling changed. Keep
   root checks serial because the Go linter uses a shared lock.
5. Inspect `git diff --numstat`; report production and tooling separately from
   tests and test support. Use the repository's `code-review` skill when an
   independent review is needed.

## Landing and continuation

Follow `AGENTS.md` for the leased worktree, protected `origin/main`, required
checks, and fetched ancestry proof. Land one coherent PR at a time. A wider
audit can continue as a separate task after refreshing from current `main`.
